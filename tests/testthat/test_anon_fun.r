...
test_that("Anonymize function is working", {
  ...
  expect_error(anonymize(df = df_exmple, info = NULL)) # "df should be a data frame
  expect_error(anonymize(df = df_exmple, calss = NULL, cols_to_anon = "name")) #"cols_to_anon should be a character vector"
  expect_error(anonymize(df = df_exmple, regexp = NULL, algo = "crc32")) # algo is a string
