...
The direct figure below demonstrates gender versus the total salary paid ($thousands) for different professor positions.
...
#runShiny()
```
#![Here is a preview of what should appear when a user opens the CyChecks shiny app](/Users/lawto/Desktop/Spring2019/STAT585/finalproj/CyChecks/CyChecks/vignettes/shiny_app_screenshot.PNG)


## Conclusion

From the CyChecks package, a user is equipped with four different functions that can help them work through the above specified Iowa salary data in order to run analysis and cross-compare wages amoungst Iowa State University departments, while investigating gender and career position equity. This package, CyChecks, allows the user to scrap the dataset from the web, anonymize the names of invidivudals to mask identity for the sake of privacy, select anf filter specific professor data, and run basic statistical analysis against the data for years 2007 to 2018.

Across the board, when graphically visualizing the data, it can be seen that there is a desparity between men and women's average yearly salary in STEM departments (i.e. Agronomy, Engineering, Mathematics) where there is a defined scarcity in female representation across these departments' academic professional careers (i.e. professor, assistance professor, professor emaritus, etc.). Conversely, it can been seen in some departments like the social sciences(i.e. education, language) within the university are majority female. Either way, this data visualization can make it easier for users to understand and interpret the trends in the data, while identifying where an initiative to close the pay gap within departments with a lack of diversity and gender  representation at each position level offered.
...
Here is a [link](CyChecks.pdf) to the CyChecks PPT that could provide more insight to users of the package.

## Thank you!

![CyChecks Hexsticker](/Users/lawto/Desktop/Spring2019/STAT585/finalproj/CyChecks/CyChecks/vignettes/CyChecks_hexsticker.png)
